...
def main(args):
    ...
    targets_all, norm_info = build_targets_and_normalizers(df)

    df_eval = df.sample(min(args.n_generate, len(df)), random_state=args.seed)
    prompts = df_eval["product"].tolist()
    targets_eval, _ = build_targets_and_normalizers(df_eval)
    ...
    # 🔹 Save valid molecules with recomputed properties
    valid_df_path = out_dir / "valid_generated_molecules.csv"
